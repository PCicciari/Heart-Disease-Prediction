...
ETModel = ExtraTreesClassifier(n_estimators=100, random_state=rng)

basicModel = ETModel.fit(X_under, y_under)

Y_pred = basicModel.predict(X_under)

print("Basic Model Recall: ", metrics.recall_score(y_under, Y_pred))
print("Basic Model Precision: ", metrics.precision_score(y_under, Y_pred))
print("Basic Model Accuracy: ", metrics.accuracy_score(y_under, Y_pred))
print("Basic Model F1 Score: ", metrics.f1_score(y_under, Y_pred))
print("------------------------------------------------------------------")
print("------------------------------------------------------------------")
print("------------------------------------------------------------------")
...
prediction = ETModel.predict(X_validate)
...
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=ETModel.classes_)
...
param_dist = {
    'n_estimators': [100, 150, 200, 250],                  # Try different maximum depths or no limit
    'criterion': ["gini", "entropy", "log_loss"],              # Minimum samples required to split a node
    'max_depth': [1, 2, 4, 6],                  # Minimum samples required at a leaf node
    'min_samples_split': [2, 3, 4],
    'min_samples_leaf': [1, 2, 3],                  # Use Gini impurity or Information Gain (Entropy)
...
optimized_etModel = ExtraTreesClassifier(
    ...
    min_weight_fraction_leaf=best_hyperparameters['min_weight_fraction_leaf'],
...
optimized_etModel.fit(X_under, y_under)
...
test_prediction = ETModel.predict(X_test)
...
y_test_prob = optimized_etModel.predict_proba(X_test)[:, 1]  # Probability for the positive class
fpr_test, tpr_test, threshold_test = roc_curve(y_test, y_test_prob)
roc_auc_test = auc(fpr_test, tpr_test)

# Display the confusion matrix of the Validation Test:
disp = ConfusionMatrixDisplay(confusion_matrix=cm_test, display_labels=optimized_etModel.classes_)
disp.plot(cmap=plt.cm.Blues)
