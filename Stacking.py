...
DTModel = DecisionTreeClassifier(min_samples_split=20, min_samples_leaf=2, max_features='log2', max_depth=7, criterion='gini', random_state=rng)
...
DTfit = DTModel.fit(X_under, y_under)
...
estimators = [('ET', make_pipeline(StandardScaler(), ETModel)), ('Ada', make_pipeline(StandardScaler(), AdaModel)),
              ('DT', make_pipeline(StandardScaler(), DTModel)), ('RF', make_pipeline(StandardScaler(), RFModel))]

# Stacking model initialization and accuracy before hyperparam tuning
StackingModel = StackingClassifier(estimators=estimators, final_estimator=LogisticRegression(random_state=rng))
...
optimized_Stacking = StackingClassifier(
    ...
    final_estimator=LogisticRegression(random_state=rng),
